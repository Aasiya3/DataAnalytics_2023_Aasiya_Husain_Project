...
#EMS<-readRDS("Data Analytics/files/EMS Data/training_final.RDS")
EMS<-readRDS("EMS Data/training_final.RDS")

#label initial_call_type
val_lab(EMS$initial_call_type)=num_lab("
...
data.frame(table(EMS$initial_call_type))

#label final_call_type
val_lab(EMS$final_call_type)=num_lab("
1 Abdominal Pain-Fever & Cough
2 Abdominal Pain-Fever & Cough
3 Abdominal Pain Fever-Travel
4 Abdominal Pain
...
data.frame(table(EMS$final_call_type))

#create month variable
EMS$month <- format(as.Date(EMS$incident_dt), "%m")

#create day of week variable
EMS$dow <- weekdays(as.Date(EMS$incident_dt))

#label incident disposition code
val_lab(EMS$incident_disposition_code)=num_lab("
...
#check that final call type labels loaded correctly
data.frame(table(EMS$incident_disposition_code))

#run table of incident disposition code by year
table(EMS$incident_disposition_code, EMS$incident_year)



#take out rows where BOROUGH = Brooklyn
EMS_Brooklyn <- EMS[EMS$borough == "BROOKLYN",]

counts_total <- table(EMS_Brooklyn$incident_year) #creates a count of incidents by year and stores it in a new value frame
barplot(counts_total, xlab="Year", ylab="# of Incidents", col=c("red"), main = "EMS Incidents by Year")
#It is still a little slow but much faster the EMS. It is about 3 million rows.
...
counts_total <- table(EMS$incident_year) #creates a count of incidents by year and stores it in a new value frame
  barplot(counts_total, xlab="Year", ylab="# of Incidents", col=c("red"), main = "EMS Incidents by Year")
...
#multiple line chart of incidents by month and year
  counts_month <- as.data.frame.matrix(table(EMS$month,EMS$incident_year)) #creates a count of incidents by year and stores it in a new value frame
...
#stacked bar chart  of incident dispositions by year
  counts <- table(EMS$incident_disposition_code, EMS$incident_year) #creates a count of incidents by year and stores it in a new value frame
